...
BeforeAll({ timeout: 30000 }, async function () {
  const targetUrl = process.env.SALEOR_GRAPHQL_URL || 'http://localhost:8000/graphql/';
...
Before(function () {
  ...
  actorCalled('Customer').whoCan(CallGraphQL.using(endpoint));
  actorCalled('Admin').whoCan(CallGraphQL.using(endpoint));
});
